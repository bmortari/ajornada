...
class ChatMessageInput(BaseModel):
    ...
    Accepts both frontend names (mensagem/historico/modelo/arquivos)
    and canonical names (content/history/model/attachments).
    """
    content: str = ""
    history: List[Dict[str, str]] = []  # Previous messages
    ...
    @classmethod
    def _accept_frontend_names(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if 'mensagem' in data and 'content' not in data:
                data['content'] = data.pop('mensagem')
            if 'historico' in data and 'history' not in data:
                data['history'] = data.pop('historico')
            if 'modelo' in data and 'model' not in data:
                data['model'] = data.pop('modelo')
            if 'arquivos' in data and 'attachments' not in data:
                data['attachments'] = data.pop('arquivos')
        return data


class ChatGenerateInput(BaseModel):
    """Generic input for generation from chat history.
    Accepts both frontend (historico/modelo) and canonical (history/model) names.
    """
    history: List[Dict[str, str]] = []
    model: Optional[str] = None
    attachments: Optional[List[Dict[str, Any]]] = []
    skills: Optional[List[Any]] = []  # Active skill IDs
...
class ChatInitResponse(BaseModel):
    ...
    dfd_data: Optional[Dict[str, Any]] = None  # Approved DFD data for ETP context


class RegenerarCampoInput(BaseModel):
    """Generic input for field regeneration.
    Accepts frontend name 'instrucao' mapped to 'prompt_adicional',
    'modelo' mapped to 'model', and 'arquivos' mapped to 'attachments'.
    """
    campo: str
    valor_atual: Optional[str] = None
    prompt_adicional: Optional[str] = None
    ...
    def _accept_frontend_names(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if 'instrucao' in data and 'prompt_adicional' not in data:
                data['prompt_adicional'] = data.pop('instrucao')
            if 'modelo' in data and 'model' not in data:
                data['model'] = data.pop('modelo')
            if 'arquivos' in data and 'attachments' not in data:
                data['attachments'] = data.pop('arquivos')
