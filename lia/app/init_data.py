...
# Contexto de hashing de senha (compatível com fastapi-users 14+ / pwdlib)
...
async def criar_prompt_templates(db):
    """Cria prompt templates padrão se não existirem"""
    from .models.prompt_template import PromptTemplate
    
    # Verificar se já existem prompts
    result = await db.execute(select(PromptTemplate))
    if result.scalars().first():
        logger.info("Prompt templates já existem. Pulando seed.")
        return
    
    logger.info("Criando prompt templates do sistema...")
    ...
    try:
        ...
        try:
            ...
            logger.warning("Nenhum arquivo seed de prompts encontrado. Pulando.")
            return
    ...
    for prompt_data in SEED_PROMPTS:
        template = PromptTemplate(
            ...
            versao=prompt_data.get("versao", "1.0"),
            ativa=prompt_data.get("ativa", True),
            ordem=prompt_data.get("ordem", 0),
            descricao=prompt_data.get("descricao", ""),
        )
        db.add(template)
        count += 1
    
    await db.commit()
    logger.info(f"Criados {count} prompt templates.")
