...
async def exportar_print(
    ...
    for campo, conf in campos_config.items():
        ...
        if valor:
            if isinstance(valor, (dict, list)):
                ...
                valor = _json.dumps(valor, ensure_ascii=False, indent=2)
            campos_html += f'''
            <div class="campo">
                <h3>{conf.get("label", campo)}</h3>
    ...
    <head>
        <meta charset="UTF-8">
        <title>{titulo} - {projeto.titulo if projeto else 'Projeto'}</title>
        <style>
    ...
    <body>
        ...
        <div class="meta">
            ...
            <strong>Versão:</strong> {artefato.versao} | <strong>Status:</strong> {artefato.status} <br>
            <strong>Data:</strong> {data_impressao}
        </div>
        {campos_html}
