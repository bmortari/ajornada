...
async def listar_pac(
    ...
    """
    logger.info(f"🟢 LISTAR PAC - Handler entered. User: {current_user.email}")
    query = select(PAC)
    ...
    try:
        ...
        logger.error(f"Erro ao consultar itens do PAC: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao consultar itens do PAC: {str(e)}"
        )
