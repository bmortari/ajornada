...
class ComprasGovService:
    ...
    # ============================================================
    # Métodos ARP - Ata de Registro de Preços
    # ============================================================
    
    async def consultar_arp_item(
        self,
        codigo_item: int,
        data_vigencia_min: Optional[str] = None,
        data_vigencia_max: Optional[str] = None,
        ...
        """
        Consulta itens de ARPs vigentes no ComprasGov.
        Endpoint: /modulo-arp/2_consultarARPItem
        
        Args:
            codigo_item: Código CATMAT/CATSERV
            data_vigencia_min: Data início vigência mínima (YYYY-MM-DD)
            data_vigencia_max: Data fim vigência máxima (YYYY-MM-DD)
            tipo_item: Tipo do item (material/servico)
            codigo_pdm: Código PDM para filtrar
            pagina: Página da consulta
            tamanho_pagina: Itens por página
            
        Returns:
            Tuple com lista de itens ARP, total de registros e total de páginas
        """
        from datetime import date as date_type
        
        endpoint = "/modulo-arp/2_consultarARPItem"
        
        # Se não informar datas, buscar ATAs vigentes (hoje ± 1 ano)
        if not data_vigencia_min:
            data_vigencia_min = date_type.today().isoformat()
        if not data_vigencia_max:
            data_vigencia_max = date_type.today().isoformat()
        
        params = {
            "codigoItem": codigo_item,
            "dataVigenciaInicialMin": data_vigencia_min,
            "dataVigenciaInicialMax": data_vigencia_max,
            "tipoItem": tipo_item,
            "codigoPdm": codigo_pdm,
            "pagina": pagina,
            "tamanhoPagina": tamanho_pagina,
        ...
        try:
            ...
            itens_validos = [
                ...
                if not item.get("itemExcluido", False)
            ]
            
            logger.info(
                f"ARP: {len(itens_validos)} itens válidos de {len(resultado)} "
                f"(total API: {total_registros}) para código {codigo_item}"
            )
            
            return itens_validos, total_registros, total_paginas
            
        except Exception as e:
            logger.error(f"Erro ao consultar ARP para código {codigo_item}: {e}")
            return [], 0, 0
    
    async def buscar_arps_vigentes(
        self,
        codigos_catmat: List[int],
        max_resultados_por_codigo: int = 10,
    ) -> List[Dict[str, Any]]:
        """
        Busca ARPs vigentes para uma lista de códigos CATMAT/CATSERV.
        Consolida resultados de múltiplos códigos.
        
        Args:
            codigos_catmat: Lista de códigos CATMAT/CATSERV
            max_resultados_por_codigo: Máximo de resultados por código
            
        Returns:
            Lista consolidada de itens ARP vigentes
        """
        todas_arps = []
        ids_vistos = set()
        
        for codigo in codigos_catmat:
            try:
                itens, _, _ = await self.consultar_arp_item(
                    codigo_item=codigo,
                    tamanho_pagina=max_resultados_por_codigo,
                )
                
                for item in itens:
                    # Deduplicar por número da ata + item
                    chave = f"{item.get('numeroAtaRegistroPreco')}-{item.get('numeroItem')}"
                    if chave not in ids_vistos:
                        ids_vistos.add(chave)
                        todas_arps.append(item)
                        
            except Exception as e:
                logger.warning(f"Erro ao buscar ARP para CATMAT {codigo}: {e}")
                continue
        
        # Ordenar por valor unitário (menor primeiro)
        todas_arps.sort(key=lambda x: x.get("valorUnitario") or float("inf"))
        
        logger.info(f"Total ARPs vigentes encontradas: {len(todas_arps)} para {len(codigos_catmat)} códigos")
        return todas_arps
