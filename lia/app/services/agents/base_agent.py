...
class BaseAgent(ABC):
    ...
    async def _load_prompt(self):
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                self.system_prompt = await loader.get_prompt(
                    ...
                    prompt_type="system",
                    default=self.system_prompt or None
                )
        ...
        # Fallback genérico se prompt ainda estiver vazio
        if not self.system_prompt:
            self.system_prompt = (
                "Você é um Especialista em Contratações Públicas (Lei 14.133/2021). "
                "Gere o artefato solicitado em formato JSON válido. "
                "Use linguagem formal, impessoal e objetiva. "
                "Retorne APENAS o JSON puro, sem markdown, sem explicações."
            )
            logger.info(f"[{self.__class__.__name__}] Usando system_prompt fallback")
