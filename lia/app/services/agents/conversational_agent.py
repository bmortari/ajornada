...
class ConversationalAgent:
    ...
    async def _load_prompts(self):
        ...
        # Fallback: se prompts ainda estiverem vazios, usar defaults genéricos
        if not self.system_prompt_chat:
            self.system_prompt_chat = (
                f"Você é a LIA, assistente do TRE-GO para elaboração de documentos de contratação pública "
                f"conforme a Lei 14.133/2021. Você está ajudando a elaborar o artefato: {self.nome_artefato}. "
                f"Converse naturalmente com o usuário para coletar as informações necessárias. "
                f"Quando tiver informações suficientes, faça um resumo e adicione a tag "
                f"[GERAR_{self.nome_artefato.upper().replace(' ', '_')}] ao final da mensagem."
            )
            logger.info(f"[{self.__class__.__name__}] Usando system_prompt_chat fallback")
        
        if not self.system_prompt_generate:
            self.system_prompt_generate = (
                f"Você é um Especialista em Contratações Públicas (Lei 14.133/2021). "
                f"Gere o artefato '{self.nome_artefato}' completo em formato JSON válido. "
                f"Use linguagem formal, impessoal e objetiva. "
                f"Retorne APENAS o JSON puro, sem markdown, sem explicações."
            )
            logger.info(f"[{self.__class__.__name__}] Usando system_prompt_generate fallback")
    
    def get_tools(self, context: ChatContext) -> List[Dict[str, Any]]:
        """Retorna a lista de ferramentas disponíveis para o agente neste contexto."""
        return [{
            "type": "function",
            "function": {
                "name": "generate_artifact",
                "description": f"Sinaliza que todas as informações foram coletadas e o {self.nome_artefato} deve ser gerado. Chame esta ferramenta assim que o usuário confirmar."
            }
        }]
...
🌐 IDIOMA OBRIGATÓRIO: Sempre pense, raciocine e responda em português brasileiro. Nunca use outro idioma, nem mesmo no raciocínio interno.
...
5. OBRIGATÓRIO: Se o usuário confirmar a geração, utilize OBRIGATORIAMENTE a ferramenta 'generate_artifact' para sinalizar que a coleta acabou e o documento deve ser gerado.
6. Sempre responda com uma frase natural confirmando a ação ANTES ou JUNTO de chamar a ferramenta.
...
8. Seja conciso e objetivo nas perguntas"""
    ...
    async def chat(
        ...
        # --- EXTRAÇÃO DE GESTOR/FISCAL DO CHAT ---
        def extract_role(text, role):
            # Accepts: "gestor: X", "fiscal: Y", "Gestor do contrato: Z"
            pattern = rf"{role}\s*:?\s*([A-Za-zÀ-ÿ ']+)"
            ...
            gestor_val = extract_role(message, "gestor")
            fiscal_val = extract_role(message, "fiscal")
            if gestor_val:
                context.dados_coletados["responsavel_gestor"] = gestor_val
            if fiscal_val:
                context.dados_coletados["responsavel_fiscal"] = fiscal_val

            # Optionally scan history for missing values
            if not context.dados_coletados.get("responsavel_gestor") or not context.dados_coletados.get("responsavel_fiscal"):
                for msg in reversed(history):
                    if msg.role == "user":
                        if not context.dados_coletados.get("responsavel_gestor"):
                            val = extract_role(msg.content, "gestor")
                            if val:
                                context.dados_coletados["responsavel_gestor"] = val
                        if not context.dados_coletados.get("responsavel_fiscal"):
                            val = extract_role(msg.content, "fiscal")
                            if val:
                                context.dados_coletados["responsavel_fiscal"] = val
        ...
        try:
            ...
            async for chunk in stream:
                ...
                # TRUE STREAMING: yield ONLY the incremental chunk from this delta
                # DO NOT accumulate - send each piece immediately as it arrives

                # Reasoning chunks (OpenRouter extended thinking)
                ...
                if not reasoning_chunk and hasattr(delta, "model_extra") and delta.model_extra:
                    reasoning_chunk = delta.model_extra.get("reasoning") or delta.model_extra.get("reasoning_content")

                if reasoning_chunk:
                    # Yield ONLY this chunk (not accumulated)
                    yield {"type": "reasoning", "content": reasoning_chunk}

                # Content chunks (normal text response)
                if delta.content:
                    # Yield ONLY this chunk (not accumulated)
                ...
                if delta.tool_calls:
                    for tc in delta.tool_calls:
                        if tc.function:
                            if tc.function.name == "generate_artifact":
                                yield {"type": "action", "action": "generate"}
                            elif tc.function.name == "search_arp_tool":
                                yield {
                                    "type": "action", 
                                    "action": "search_arp", 
