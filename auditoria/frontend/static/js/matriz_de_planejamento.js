document.addEventListener('DOMContentLoaded', () => {
    ...
    const campoMatrizRiscos = document.getElementById('upload-matriz-riscos');
    const uploadAreas = document.querySelectorAll('.cursor-pointer input.file-input');
    
    // Botões
    const btnGerarMatriz = document.getElementById('btn-gerar-matriz-planejamento');
    const containerBotoesVisualizar = document.getElementById('btn-container-visualizar');
    const btnVisualizarMatriz = document.getElementById('btn-visualizar-matriz-planejamento');
...
<body>
    ...
    btnGerarMatriz.addEventListener('click', () => {
        ...
        if(campoPlanoTrabalho.files[0]) formData.append('plano_trabalho', campoPlanoTrabalho.files[0]);
        if(campoMatrizRiscos.files[0]) formData.append('matriz_riscos', campoMatrizRiscos.files[0]);
        formData.append('prompt_usuario', campoInstrucoes.value);

        fetch(`${window.location.origin}/gerar-matriz-planejamento`, {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.reply) {
                conteudoMatrizGerado = limparHTMLGerado(data.reply);
    ...
    btnVisualizarMatriz.addEventListener('click', () => {
        if (conteudoMatrizGerado) abrirCuradoria('matriz');
    ...
    if(btnSalvarCuradoria){
        btnSalvarCuradoria.addEventListener('click', () => {
            ...
            if (documentoEmCuradoria === 'matriz') {
                conteudoMatrizGerado = conteudoEditado;
                exibirPrevia(conteudoMatrizGerado);
            }
        });
    }
    ...
    function abrirCuradoria(tipo) {
        ...
        const conteudo = conteudoMatrizGerado;
    ...
    function validarArquivos() {
        if (!campoPlanoLongoPrazo.files[0] && !campoPlanoAnual.files[0] && !campoPlanoTrabalho.files[0] && !campoMatrizRiscos.files[0]) {
    ...
    function exibirPrevia(html) {
        ...
        configurarBotaoDownload(document.getElementById('btn-download'), html, 'matriz-de-planejamento.pdf', 'Landscape');
