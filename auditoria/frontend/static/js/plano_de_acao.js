document.addEventListener('DOMContentLoaded', () => {
    ...
    const campoMatrizAchados = document.getElementById('upload-matriz-achados');
    ...
    // Botões
    const btnGerarPlanoAcao = document.getElementById('btn-gerar-plano-acao');
    const containerBotoesVisualizar = document.getElementById('btn-container-visualizar');
    const btnVisualizarPlanoAcao = document.getElementById('btn-visualizar-plano-acao');
    ...
    // --- VARIÁVEIS DE ESTADO ---
    let conteudoPlanoAcaoGerado = null;
    let documentoEmCuradoria = null;

    // --- INICIALIZAÇÃO DOS MODAIS (Flowbite) ---
    let instanciaModalPlanoAcao = null;
...
<body>
    ...
    function resetarInterface() {
        ...
        btnGerarPlanoAcao.classList.remove('hidden');
        campoInstrucoes.value = '';
        uploadAreas.forEach(input => {
            input.value = '';
            input.dispatchEvent(new Event('change', { 'bubbles': true }));
        });
        conteudoPlanoAcaoGerado = null;
        documentoEmCuradoria = null;
        btnGerarPlanoAcao.disabled = false;
    ...
    // ==================================================================
    const modalPlanoAcaoEl = document.getElementById('modal-previa-plano-acao');
    if (modalPlanoAcaoEl) instanciaModalPlanoAcao = new Modal(modalPlanoAcaoEl);
    
    if (modalCuradoriaEl) instanciaModalCuradoria = new Modal(modalCuradoriaEl, { closable: false, backdrop: 'static' });
    
    ['modal-previa-plano-acao', 'modal-curadoria'].forEach(id => {
        document.querySelectorAll(`[data-modal-hide="${id}"]`).forEach(button => {
            button.addEventListener('click', () => {
                const instance = [instanciaModalPlanoAcao, instanciaModalCuradoria].find(inst => inst && inst._targetEl.id === id);
                if (instance) instance.hide();
            });
        });
    });

    btnGerarPlanoAcao.addEventListener('click', () => {
        if (!validarArquivos()) return;
        exibirLoading(true);
        btnGerarPlanoAcao.disabled = true;

        const formData = new FormData();
        formData.append('matriz_achados', campoMatrizAchados.files[0]);
        if (campoMatrizPlanejamento.files[0]) {
        ...
        fetch(`${window.location.origin}/aud/plano-acao`, {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.reply_acao) {
                conteudoPlanoAcaoGerado = limparHTMLGerado(data.reply_acao);
                btnGerarPlanoAcao.classList.add('hidden');
    ...
    btnVisualizarPlanoAcao.addEventListener('click', () => {
        if (conteudoPlanoAcaoGerado) abrirCuradoria('plano-acao');
        else exibirErro('Conteúdo do plano de ação não disponível.');
    });
    ...
    if(btnSalvarCuradoria){
        btnSalvarCuradoria.addEventListener('click', () => {
            ...
            if (documentoEmCuradoria === 'plano-acao') {
                conteudoPlanoAcaoGerado = conteudoEditado;
                exibirPreviaPlanoAcao(conteudoPlanoAcaoGerado);
            }
        });
    }
    ...
    function abrirCuradoria(tipo) {
        ...
        const conteudo = conteudoPlanoAcaoGerado;
    ...
    function validarArquivos() {
        if (!campoMatrizAchados.files[0]) {
            exibirErro('Por favor, anexe a Matriz de Achados para prosseguir.');
            return false;
        }
        return true;
    }
    ...
    function exibirPreviaPlanoAcao(html) {
        renderizarConteudoSanitizado(document.getElementById('conteudo-previa-plano-acao'), html);
        configurarBotaoDownload(document.getElementById('btn-download-plano-acao'), html, 'plano-de-acao.html', 'Portrait');
        instanciaModalPlanoAcao.show();
    }
