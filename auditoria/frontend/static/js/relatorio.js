document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- VARIÁVEIS DE ESTADO ---
    let conteudoRelatorioGerado = null;
    let documentoEmCuradoria = null;

    // --- INICIALIZAÇÃO DOS MODAIS (Flowbite) ---
    let instanciaModalRelatorio = null;
...
<body>
    ...
    // ==================================================================
    const modalRelatorioEl = document.getElementById('modal-previa-relatorio');
    if (modalRelatorioEl) instanciaModalRelatorio = new Modal(modalRelatorioEl);
    
    if (modalCuradoriaEl) instanciaModalCuradoria = new Modal(modalCuradoriaEl, { closable: false, backdrop: 'static' });
    
    ['modal-previa-relatorio', 'modal-curadoria'].forEach(id => {
        document.querySelectorAll(`[data-modal-hide="${id}"]`).forEach(button => {
            button.addEventListener('click', () => {
                const instance = [instanciaModalRelatorio, instanciaModalCuradoria].find(inst => inst && inst._targetEl.id === id);
    ...
    btnGerarRelatorio.addEventListener('click', () => {
        ...
        fetch(`${window.location.origin}/aud/relatorio`, {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.reply) {
                conteudoRelatorioGerado = limparHTMLGerado(data.reply);
    ...
    btnVisualizarRelatorio.addEventListener('click', () => {
        if (conteudoRelatorioGerado) abrirCuradoria('relatorio');
        else exibirErro('Conteúdo do relatório não disponível.');
    });
    ...
    if(btnSalvarCuradoria){
        btnSalvarCuradoria.addEventListener('click', () => {
            ...
            if (documentoEmCuradoria === 'relatorio') {
                conteudoRelatorioGerado = conteudoEditado;
                exibirPreviaRelatorio(conteudoRelatorioGerado);
            }
        });
    }
    ...
    function abrirCuradoria(tipo) {
        ...
        const conteudo = conteudoRelatorioGerado;
        renderizarConteudoSanitizado(editorCuradoria, conteudo);
        editorCuradoria.contentEditable = "false";
        instanciaModalCuradoria.show();
    }

    function validarArquivos() {
        if (!campoPlanoTrabalho.files[0] || !campoMatrizPlanejamento.files[0] || !campoMatrizAchados.files[0]) {
            exibirErro('Por favor, anexe todos os três arquivos para prosseguir.');
            return false;
        }
        return true;
    }
    ...
    function exibirPreviaRelatorio(html) {
        const iframe = document.createElement('iframe');
        iframe.style.width = '100%';
        iframe.style.height = '70vh';
        iframe.style.border = 'none';
        iframe.srcdoc = html;
        const container = document.getElementById('conteudo-previa-relatorio');
        container.innerHTML = '';
        container.appendChild(iframe);
        configurarBotaoDownload(document.getElementById('btn-download-relatorio'), html, 'relatorio.html', 'Portrait');
        instanciaModalRelatorio.show();
    }
    
    function sanitizarHTML(html) {
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = html;
        tempDiv.querySelectorAll('script').forEach(el => el.remove()); // Keep style/link for editor
        return tempDiv.innerHTML;
    }

    function renderizarConteudoSanitizado(container, conteudo) {
        // For editor, extract body content to avoid nesting issues, but preserve styles
        const parser = new DOMParser();
        const doc = parser.parseFromString(conteudo, 'text/html');
        let bodyContent = doc.body.innerHTML;
        
        // Prepend head styles if present
        const head = doc.head;
        const styles = [];
        head.querySelectorAll('style, link[rel="stylesheet"]').forEach(styleEl => {
            styles.push(styleEl.outerHTML);
        });
        if (styles.length > 0) {
            bodyContent = styles.join('\n') + '\n' + bodyContent;
        }
        
        container.innerHTML = sanitizarHTML(bodyContent);
    }
